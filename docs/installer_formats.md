...
| **InstallShield** | ✅ v0.2.3 | 自研（移植 unshield 核心公式） | ASUS ROG data1.hdr (55 dirs, 260 files, 49 executables) |
| **Squirrel (Electron)** | ✅ v0.2.3 | 自研（EOCD tail scan + OffsetFile） | Claude Desktop 130MB (0.28s detect) |
| **Node.js/Bun SEA** | ✅ v0.2.3 | 自研（PE section table direct read） | Claude Code 243MB, .bun section 121MB (0.30s detect) |

### 全部完成

所有 13 种格式均已实现并验证。无待支持项。

InstallShield 通过移植 unshield 核心算法（3 个关键公式）实现，200 行纯 Python，无 C 依赖。
...
- **格式**: CAB-based（签名 `0x28635349` = "ISc("）
- **版本**: V5（0x3A 字节/descriptor）和 V6+（0x57 字节/descriptor）
- **关键差异**:
  - V5: file_table 数组索引文件 → `desc_off + ft_off + file_table[dir_count + i]`
  - V6+: 线性排列 → `desc_off + ft_off + ft_off2 + i * 0x57`
  - V6+: descriptor 在 `.hdr` 文件中（.cab 的 desc_size=0）
- **参考工具**:
  - [unshield](https://github.com/twogood/unshield) — C 库（核心公式来源）
  - [Observer ishield 模块](https://github.com/lazyhamster/Observer) — C++
  - [SFUnpacker](https://github.com/Puyodead1/SFUnpacker) — C++（Setup Factory 用，源自 Observer）
- **ppm 实现**: `adapters/ishield.py` — 200 行纯 Python，移植 unshield 的 3 个关键公式：
  1. CommonHeader 解析（签名 + desc_offset + desc_size）
  2. CabDescriptor 字段偏移（+0x0C ft_off, +0x1C dir_count, +0x28 file_count, +0x2C ft_off2）
  3. FileDescriptor 读取（V5 file_table 索引 vs V6 线性排列）
- **Mythos 思维**: 不需要整个 unshield C 库 + CMake + ctypes binding，只要 3 个公式
...
- **创建者**: GitHub (Squirrel.Windows)
- **格式**: PE32 stub + 嵌入的 .nupkg (NuGet = ZIP) 在 .rsrc section 中
- **特征**: PE 中含 `SquirrelInstall` / `Squirrel` UTF-16LE 字符串 + 巨大的 .rsrc section
- **用户**: Discord, Slack, VS Code, Claude Desktop, 1Password, Signal Desktop
- **ppm 实现**: `adapters/squirrel.py`
  - 快速检测：200KB header 字符串扫描（不用 pefile）
  - nupkg 提取：EOCD tail scan → OffsetFile wrapper（不加载整个文件到内存）
  - 解析 .nuspec XML 提取应用名/版本
- **性能优化**: 130MB Claude Desktop detect 0.28s（优化前 5.6s，20x 加速）

### Node.js / Bun SEA (Single Executable Application)

**已实现。**

- **创建者**: Node.js (node:sea) / Oven (Bun)
- **格式**: PE + 嵌入的 JavaScript 应用
  - Node.js SEA: 通过 postject 注入 NODE_SEA_BLOB 资源
  - Bun SEA: `.bun` PE section 包含 bundled JavaScript
- **特征**:
  - Bun: PE section table 中有 `.bun` section（通常 100MB+）
  - Node: PE 资源区含 `NODE_SEA_BLOB` 或 `postject` 标记
- **用户**: Claude Code (Bun SEA), 各种 CLI 工具
- **ppm 实现**: `adapters/node_sea.py`
  - 快速检测：直接读 PE header section table（e_lfanew → section names），不用 pefile
  - 解析：section 列表 + import 表 + 版本字符串提取
- **性能优化**: 243MB Claude Code detect 0.30s（优化前 10.6s，35x 加速）
- **彩蛋**: ppm 分析了自己（Claude Code）的安装包——`.bun` section 121MB 就是"我"
...
| ishield | InstallShield | ✅ 已实现 | 移植 unshield 核心公式（V5 file_table + V6 linear） |
...
| InstallShield | ASUS ROG data1.hdr | eSupport | ✅ INSTALLSHIELD | ✅ 55 dirs, 260 files, 2 patterns |
| Squirrel | Claude Desktop 130MB | Downloads | ✅ SQUIRREL | ✅ 0.28s detect |
| Node/Bun SEA | Claude Code 243MB | .local/bin | ✅ NODE_SEA | ✅ .bun 121MB, 0.30s detect |
...
6. **抓骨架不遍历** — 大文件（100MB+）只读 header/tail 定位格式，不做全量解析

### 性能基准（detect 阶段）

| 文件大小 | 格式 | 优化前 | 优化后 | 方法 |
|---------|------|--------|--------|------|
| 243 MB | Bun SEA | 10.6s | 0.30s | PE section table 在 header 前几百字节 |
| 130 MB | Squirrel | 5.6s | 0.28s | 200KB header 字符串扫描 |
| 137 KB | NSIS | 0.5s | 0.32s | 无需优化 |
| — | MSI | 0.3s | 0.28s | OLE2 magic 4 字节 |

**核心洞察**: PE section table 位于 `e_lfanew + 24 + optional_header_size`，
通常在文件前 1KB 内。读 200KB header 就能判断所有 15 种格式，不需要 pefile 解析整个文件。
